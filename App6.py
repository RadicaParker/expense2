...
def init_db():
    ...
    if admin_count == 0:
        exec_sql(
            "INSERT INTO users (email, name, password, role, approver_email) VALUES (?, ?, ?, ?, ?)",
...
def create_user(email, name, password, role="user", approver_email=""):
    try:
        exec_sql(
            "INSERT INTO users (email, name, password, role, approver_email) VALUES (?, ?, ?, ?, ?)",
            (email, name, hash_pw(password), role, approver_email)
        )
...
def get_user_approver(user_email):
    row = fetch_one("SELECT approver_email FROM users WHERE email = ?", (user_email,))
    if row and row[0]:
        return row[0]
    return ""
...
if not st.session_state.logged_in:
    ...
    if menu == "Expense Form":
        ...
        with st.form("expense_form"):
            ...
            if save_btn:
                if amount <= 0:
                    ...
                    approver_email = get_user_approver(st.session_state.user_email)
                    ...
                    if approver_email == "":
                        ...
                        exec_sql(
                            "INSERT INTO expenses (expense_date, user_email, amoeba, category, description, amount, payment_method, receipt_name, status, approver_comment, approved_by, assigned_approver) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                            (
                                str(expense_date),
                                st.session_state.user_email,
                                amoeba,
        ...
        df = fetch_df(
            "SELECT id, expense_date, amoeba, category, description, amount, payment_method, receipt_name, status, assigned_approver, approver_comment, approved_by FROM expenses WHERE user_email = ? ORDER BY id DESC",
            (st.session_state.user_email,)
        )
        ...
        pending_df = fetch_df(
            "SELECT id, expense_date, user_email, amoeba, category, description, amount, payment_method, receipt_name, status FROM expenses WHERE status = ? AND assigned_approver = ? ORDER BY id DESC",
        ...
        if pending_df.empty:
            ...
            option_map = {
                f"ID {row['id']} | {row['user_email']} | ${row['amount']:.2f} | {row['category']}": row["id"]
            ...
            selected_df = pending_df[pending_df["id"] == selected_id]
        ...
        users_df = fetch_df("SELECT id, email, name, role, approver_email FROM users ORDER BY id")
        st.dataframe(users_df, use_container_width=True)

        approver_choices = [""] + users_df["email"].tolist()
        ...
        with st.form("add_user_form"):
            ...
            new_approver = st.selectbox("Assigned Approver", approver_choices)
            ...
            if add_user_btn:
                if not new_name or not new_email or not new_password:
                    ...
                    ok, msg = create_user(new_email, new_name, new_password, new_role, new_approver)
                    if ok:
                        st.success(msg)
                        st.rerun()
                    else:
        ...
        st.markdown("### Update User Approver")
        ...
        if not non_admin_df.empty:
            ...
            approver_index = 0
            if current_approver in approver_choices:
                approver_index = approver_choices.index(current_approver)

            selected_approver = st.selectbox("Select approver", approver_choices, index=approver_index, key="approver_update")

            if st.button("Update Approver"):
                exec_sql(
                    "UPDATE users SET approver_email = ? WHERE email = ?",
                    (selected_approver, selected_user_email)
                )
                st.success("Approver updated.")
                st.rerun()
