...
if not st.session_state.logged_in:
    ...
    if menu == "Expense Form":
        ...
        with st.form("expense_form"):
            ...
            if save_btn:
                if amount <= 0:
                    ...
                    exec_sql(
                        "INSERT INTO expenses (expense_date, user_email, amoeba, category, description, amount, payment_method, receipt_name, status, approver_comment, approved_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                        (
                            str(expense_date),
                            st.session_state.user_email,
                            amoeba,
        ...
        df = fetch_df(
            "SELECT id, expense_date, amoeba, category, description, amount, payment_method, receipt_name, status, approver_comment, approved_by FROM expenses WHERE user_email = ? ORDER BY id DESC",
            (st.session_state.user_email,)
        )
    ...
    elif menu == "Approval Queue" and st.session_state.user_role == "admin":
        st.subheader("Approval Queue")

        pending_df = fetch_df(
            "SELECT id, expense_date, user_email, amoeba, category, description, amount, payment_method, receipt_name, status FROM expenses WHERE status = ? ORDER BY id DESC",
            ("Submitted",)
        )
